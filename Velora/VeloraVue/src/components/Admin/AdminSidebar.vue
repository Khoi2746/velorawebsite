    ...
    <ul class="sidebar-menu">
      ...
      <li class="menu-group" :class="{ 'menu-open': openGroups.inventory }">
        ...
        <div class="sub-menu-collapse">
          <ul class="sub-menu">
            <template v-for="item in getGroupItems(['Quản Lý Sản Phẩm', 'Quản Lý Loại Sản Phẩm', 'Quản Lý Thương Hiệu', 'Quản Lý Kho', 'Phiếu Nhập Kho'])" :key="item.name">
              <li>
      ...
      <li class="menu-group" :class="{ 'menu-open': openGroups.sales }">
        ...
        <div class="sub-menu-collapse">
          <ul class="sub-menu">
            <template v-for="item in getGroupItems(['Quản Lý Đơn Đặt', 'Quản Lý Hoàn Tiền', 'Xuất Hóa Đơn', 'Quản Lý Mã Giảm Giá', 'Thống Kê Doanh Thu'])" :key="item.name">
              <li>
                <router-link :to="item.link" active-class="active">
                  <i :class="item.icon"></i>
                  <span>{{ item.name }}</span>
      ...
      <li class="menu-group" :class="{ 'menu-open': openGroups.operations }">
        ...
        <div class="sub-menu-collapse">
          <ul class="sub-menu">
            <template v-for="item in getGroupItems(['Quản Lý Người Dùng', 'Quản Lý Lịch Hẹn', 'Quản Lý Bảo Hành', 'Tư Vấn Khách Hàng'])" :key="item.name">
              <li>
                <router-link :to="item.link" active-class="active">
                  <i :class="item.icon"></i>
                  <span>{{ item.name }}</span>
...
.user-panel { display: flex; padding: 20px 15px; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.05); }
...
.sidebar-menu a { display: flex; align-items: center; padding: 10px 15px; color: #dedede; text-decoration: none; transition: 0.2s; border-radius: 6px; font-size: 13.5px; }
.sidebar-menu a:hover, .sidebar-menu a.active { color: var(--gold-matte); background: var(--wood-active); }
.sidebar-menu i { width: 28px; font-size: 15px; color: inherit; }
...
.arrow-icon {
  ...
  display: inline-block; /* BẮT BUỘC PHẢI CÓ DÒNG NÀY */
  opacity: 0.5;
}

.menu-open .arrow-icon {
  transform: rotateX(180deg); /* Lật 3D theo trục X */
  opacity: 1;
}
...
.sub-menu {
  min-height: 0;
  overflow: hidden; /* Bắt buộc để ẩn khi grid 0fr */
  list-style: none;
  padding-left: 15px;
  margin-left: 15px;
...
.sidebar-collapsed .sidebar-menu span, /* <-- THÊM DÒNG NÀY ĐỂ ẨN CHỮ CỦA TRANG QUẢN TRỊ */
.sidebar-collapsed .arrow-icon, 
.sidebar-collapsed .sub-menu-collapse { 
  display: none !important; 
}
.sidebar-collapsed .logo-mini { display: block; color: var(--gold-matte); font-weight: 900; font-size: 26px; text-align: center; width: 100%; text-shadow: 0 0 10px rgba(209,170,104,0.3); }
.sidebar-collapsed .user-panel { padding: 15px 0; justify-content: center; background: transparent; border: none; }
.sidebar-collapsed .user-panel .image img { margin-right: 0; width: 45px; height: 45px; }
.sidebar-collapsed .sidebar-menu { padding: 10px; }
.sidebar-collapsed .menu-group-header { justify-content: center; padding: 15px 0; }
.sidebar-collapsed .group-title span { display: none; }
.sidebar-collapsed .group-title i { margin: 0; font-size: 20px; }
/* =======================================================
