...
const printMode = ref('month'); // 'month' hoặc 'year'
...
// Hàm kích hoạt in theo chế độ
const triggerPrint = (mode) => {
    printMode.value = mode; // Gắn cờ để Vue đổi Form
    showExportMenu.value = false; // Đóng menu lại
    
    // Chờ 0.1s để Vue kịp render sang form mới rồi mới bung máy in lên
    setTimeout(() => {
        window.print();
    }, 100);
};
...
// DỮ LIỆU MẪU CHO BẢN IN NĂM (Vì API đếm sản phẩm chưa code)
...
/* CSS CHO NÚT BẤM VÀ POPUP XUẤT BÁO CÁO */
...
.export-dropdown button:hover { background: #fdfaf5; color: #d1aa68; }
.export-dropdown button:not(:last-child) { border-bottom: 1px solid #f0efeb; }
...
.year-card .kpi-icon { background: #3e332e; color: #d1aa68; }
...
@media print {
    ...
    .print-header-company { text-align: left; font-size: 13pt; line-height: 1.6; margin-bottom: 25px; }
    .print-header-company p { margin: 2px 0; }
    .print-line-divider { width: 35%; height: 1px; background-color: #000; margin-top: 8px; }

    .print-title { text-align: center; font-size: 20pt; font-weight: bold; margin: 35px 0 10px 0; line-height: 1.3; }
    .print-subtitle { font-size: 16pt; font-weight: bold; }

    .print-meta { margin: 40px 0 20px 0; font-size: 13pt; line-height: 1.8; }
    .print-meta p { margin: 4px 0; }

    .print-table { width: 100%; border-collapse: collapse; margin-top: 15px; font-size: 13pt; }
    .print-table th, .print-table td { border: 1px solid #000; padding: 10px 12px; }
    .print-table th { background-color: #f2f2f2 !important; -webkit-print-color-adjust: exact; font-weight: bold; }

    .print-footer-summary { text-align: right; margin-top: 20px; padding-right: 10px; }
    .total-text { font-size: 14pt; font-weight: bold; }

    .print-signature-area { display: flex; justify-content: space-between; margin-top: 50px; text-align: center; font-size: 13pt; page-break-inside: avoid; }
