...
const filteredMenuItems = computed(() => {
    ...
    if (role === 'ROLE_CHUYEN_VIEN_TU_VAN') return allMenuItems.filter(item => item.name === 'Tư Vấn Khách Hàng');
    if (role === 'ROLE_ADMIN') return allMenuItems.filter(item => item.name !== 'Tư Vấn Khách Hàng');
    return []; 
});
